...
def generate(request=nil, version=nil, request_detail_fields=[])
  ...
  def add_check_http_method (file, value)
  ...
  def add_check_individual_header (file, name, value)
  ...
  File.open(filename, "w") do |file|
    ...
    requests.each do |r|
      ...
      # check http method
      add_check_http_method(file, r.http_method)
      file.puts ""
  
      # check names of the headers
      add_check_strict_headers(file, request_detail_fields)
      file.puts ""

      # check individual headers
      request_detail_fields.each do |item|; 
        unless item.keys[0] == "remarks"
          add_check_individual_header(file, map_dbname_httpname(item.keys[0]), r[item.keys[0]]) 
        end
      end
      file.puts ""
